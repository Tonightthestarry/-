...
const authAPI = {
    ...
    // 用户管理(AdminController 统一前缀 /api/admin)
...
const visualAPI = {
    ...
    // 新增图表
    trafficFlow24h(date) {
        return api.get('/api/visual/traffic-flow-24h', { params: { date: date || '' } });
    },
    trafficAnomaly(date) {
        return api.get('/api/visual/traffic-anomaly', { params: { date: date || '' } });
    },
    opinionAnomaly(date) {
        return api.get('/api/visual/opinion-anomaly', { params: { date: date || '' } });
    }
};
...
const schedulerAPI = {
    ...
    updateConfig: (autoFixedRateMs, autoCount, dailyCron, dailyCount) =>
        api.post('/api/scheduler/config', null, {
            params: { autoFixedRateMs, autoCount, dailyCron, dailyCount }
        })
};
