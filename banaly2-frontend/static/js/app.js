            ...
            chart.setOption({
                tooltip: {
                    ...
                    formatter: p => {
                        ...
                        return `${districts[i]}<br/>拥堵指数: <b>${values[i]}</b><br/>车流量: ${flows[i]}`;
                    }
                },
                ...
                visualMap: {
                    min: Math.max(0, vMin - 0.05),
                    max: Math.min(1, vMax + 0.05),
                    calculable: true,
                    orient: 'horizontal',
                    left: 'center',
                    bottom: 5,
                    textStyle: { color: '#ccc' },
                    inRange: { color: ['#50a3ba', '#5b8ff9', '#f6c022', '#f56c6c', '#d94e5d'] }
                },
                series: [{
                    type: 'bar',
                    data: values.map((v, i) => ({ value: v, name: districts[i] })),
        ...
        async function loadTrafficFlow24h() {
            try {
                ...
                let chart = echarts.init(chartDom);
                    chart.setOption({
                        tooltip: { trigger:'axis' },
                        legend: { data:['车流量','拥堵指数','平均速度'], textStyle:{color:'#ccc'}, top:0 },
                        grid: { left:55, right:25, top:30, bottom:40 },
                        xAxis: { type:'category', data:res.data.data.map(d=>d.hour), axisLabel:{color:'#ccc',fontSize:9,rotate:45} },
                        yAxis: [{ type:'value', name:'车流量' }, { type:'value', name:'指数/速度' }],
                        series: [
                            { name:'车流量', type:'bar', data:res.data.data.map(d=>d.flow), itemStyle:{color:'#5470c6'} },
                            { name:'拥堵指数', type:'line', yAxisIndex:1, data:res.data.data.map(d=>d.congestion), smooth:true, lineStyle:{color:'#ee6666'}, itemStyle:{color:'#ee6666'} },
                            { name:'平均速度', type:'line', yAxisIndex:1, data:res.data.data.map(d=>d.speed), smooth:true, lineStyle:{color:'#91cc75'}, itemStyle:{color:'#91cc75'} }
                        ]
                    });
        ...
        async function loadOpinionAnomaly() {
            try {
                ...
                let chart = echarts.init(chartDom);
                    chart.setOption({
                        tooltip: { trigger:'axis' },
                        legend: { data:['热度指数','负面比例'], textStyle:{color:'#ccc'}, top:0 },
                        grid: { left:55, right:25, top:30, bottom:50 },
                        xAxis: { type:'category', data:res.data.data.map(d=>d.district), axisLabel:{color:'#ccc',fontSize:9,rotate:30} },
                        yAxis: [{ type:'value', name:'热度' }, { type:'value', name:'比例%' }],
                        series: [
                            { name:'热度指数', type:'bar', data:res.data.data.map(d=> ({
                                value: d.hot_index,
                                itemStyle: { color: d.anomaly ? '#ff4757' : '#ffa502' }
                            })) },
                            { name:'负面比例', type:'line', yAxisIndex:1, data:res.data.data.map(d=>d.negative_ratio), smooth:true, lineStyle:{color:'#ff4757'}, itemStyle:{color:'#ff4757'} }
                        ]
                    });
